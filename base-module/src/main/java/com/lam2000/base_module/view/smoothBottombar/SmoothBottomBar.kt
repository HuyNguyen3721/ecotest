    ...
    private fun obtainStyledAttributes(attrs: AttributeSet?, defStyleAttr: Int) {
        ...
        try {
            ...
            itemAnimDuration = typedArray.getInt(
                R.styleable.SmoothBottomBar_timeDuration,
...
//             Add ellipsis character to item text if it is shorted
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        // Draw indicator
        if (_displayAllText) {
            rect.left = items[itemActiveIndex].icon.bounds.left.toFloat() - resources.getDimension(
                com.intuit.sdp.R.dimen._5sdp
            )
            rect.right =
                rect.left + items[itemActiveIndex].icon.bounds.width() + paintText.measureText(items[itemActiveIndex].title) + _barSideMargins + resources.getDimension(
