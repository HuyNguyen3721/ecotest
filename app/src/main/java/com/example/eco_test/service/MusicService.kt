package com.example.eco_test.service
...
import com.example.eco_test.activity.MainActivity
import com.example.eco_test.broadcast.BroadCastActionMusic
...
class MusicService : Service() {
    ...
    var listSong  = mutableListOf<ItemSong>()
    var pathSongPlaying  = "";
    //
    ...
    override fun onCreate() {
        ...
        //
        listSong.clear()
        viewModel.songLiveData.observeForever {
            it?.let {
                listSong.add(it)
            }
        }
    ...
    private fun createNotification(valueVolume: Int? = null) {
        createChannel()
...
//                }
    ...
    private fun registerBroadcast() {
        if (broadCastActionMusic == null) {
            ...
            if(viewModel.isActiveMain.value == false){
                for (i in 0 until listSong.size) {
                    if (listSong[i].path ==pathSongPlaying) {
                        if (i - 1 < 0) {
                            val data =listSong[listSong.size - 1]
                            onPlayMusic(data.path)
                            // rename control music
                            viewModel.nameMusicSong.value =(data.songName)
                            viewModel.nameAuthorSong.value =(data.singer)
                        } else {
                            val data =listSong[i - 1]
                            onPlayMusic(data.path)
                            // rename control music
                            viewModel.nameMusicSong.value = (data.songName)
                            viewModel.nameAuthorSong.value =(data.singer)
